  ...
  try {
    ...
    return <main className="contractor-shell">
      ...
      <section className="contractor-stats">
        <article><span>Subscription</span><strong className={`subscription-state subscription-state--${tenant.access_state}`}>{tenant.access_state}</strong>{tenant.stripe_customer_id ? <ManageBillingButton /> : <small>{tenant.plan_code === "launch" ? "$149/month for 3 months, then $199/month" : "Free demo"}</small>}</article>
